...
contract Lottery is LotteryEventDefinitions {
    ...
    function resetLottery(uint _minimumStakeInWei) public payable userOnly afterGameClosed winnerTookPot {
    ...
    function getPrice() public afterGameClosed winningConstraints {
        if (winner == msg.sender) {
            if (winner.send(pot)) {
                ...
                WinnerFailedToTakeWin("Wooops, something went wrong, the winner was not able to withdraw his funds. Sorry dude, that was not planned.");
            }
        } else {
    ...
    function rescueInitialAmountIfNobodyPlayed() public userOnly afterGameClosed {
